...
country = 'US'
sample_start = dt.strptime(
    "2000-01-01", '%Y-%m-%d').date().toordinal() + 366  # estimation sample
...
# TODO: Res and Spec should be separate,
# this will be fixed after the unit tests are created
...
# Create an array of the data series that we are interested in looping through
...
for i in range(len(series)):
    ...
    CommonFactor = np.matmul(
        Res["Res"]["C"][idxSeries, :5].reshape(1, -1),
        Res["Res"]["Z"][:, :5].T
    ) * \
    ...
    fig.update_yaxes(title_text=Spec.Units[idxSeries] + " ({})".format(
        Spec.UnitsTransformed[idxSeries]), row=i+1, col=1)
