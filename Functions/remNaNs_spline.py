...
def remNaNs_spline(X, options):
    ...
    Replication files for:
     ...
     Reichlin),
    in Michael P. Clements and David F. Hendry, editors, Oxford Handbook on
    ...
    if options["method"] == 1:  # replace all the missing values
        for i in range(N):  # Loop through columns
            ...
            x_MA = lfilter(np.ones((2*k+1))/(2*k+1),
                           1,
                           np.append(
                np.append(x[0]*np.ones((k, 1)), x),
                x[-1] * np.ones((k, 1))
            )
            )
        ...
        # Returns row sum for NaN values.
        # Marks true for rows with more than 80% NaN
        ...
        for i in range(N):  # Loop for each series
            ...
            x[t1:t2+1] = CubicSpline(np.where(~isnanx)
                                     [0], x[~isnanx])(np.arange(t1, t2+1))
            isnanx = np.isnan(x)
