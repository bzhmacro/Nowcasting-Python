...
def dfm(X, Spec, threshold=1e-5, max_iter=5000):
    """ DFM()    Runs the dynamic factor model
      ...
      Input arguments:
        X: Kalman-smoothed data where missing values are replaced by their
        expectation
     ...
     Output Arguments:
       ...
       Res - structure of model results with the following fields
           . X_sm | Kalman-smoothed data where missing values are replaced by
            their expectation
           . Z | Smoothed states. Rows give time, and columns are organized
            according to Res.C.
           . C | Observation matrix. The rows correspond
             ...
             - 1-20: These columns give the factor loa dings. For example, 1-5
                  ...
                  reverse-chronological order (f^G_t, f^G_t-1, f^G_t-2,
                  f^G_t-3, f^G_t-4). Columns 6-10, 11-15, and 16-20 give
                  loadings for the second, third, and fourth blocks
                   respectively.
    ...
    Par = {}
    # Block loading structure
    Par["blocks"] = Spec.Blocks.copy()
    # Number of quarterly series
    Par["nQ"] = (Spec.Frequency == "q").sum()
    # Number of lags in autoregressive of factor (same for all factors)
    Par["p"] = 1
    Par["r"] = np.ones((1, Spec.Blocks.shape[1])).astype(np.int64)
    ...
    print(pd.DataFrame(data=Spec.Blocks,
                       index=Spec.SeriesName, columns=Spec.BlockNames))
    print("\n")
    ...
    A, C, Q, R, Z_0, V_0 = InitCond(xNaN.copy(), r.copy(
    ), p, blocks.copy(), optNaN, R_mat.copy(), q, nQ, i_idio.copy())
    ...
    while num_iter < max_iter and not converged:
        ...
        if num_iter > 2:  # Check convergence
            converged, decrease = em_converged(
                loglik, previous_loglik, threshold, 1)
    ...
    print(pd.DataFrame(Res["C"][(-1-nQ+1):, :5],
                       columns=['f1_lag0', 'f1_lag1',
                                'f1_lag2', 'f1_lag3', 'f1_lag4'],
    ...
    A_index = np.hstack([np.arange(nFactors*5, nFactors*5+nM),
                         np.arange(nFactors*5+nM, A_len, 5)])
    Q_index = np.hstack([np.arange(nFactors*5, nFactors*5+nM),
...
def InitCond(x, r, p, blocks, optNaN, Rcon, q, nQ, i_idio):
    ...
    #  - optNaN: Option for missing values in spline.
    #            See remNaNs_spline() for details.
    #  - Rcon:   Incorporates estimation for quarterly series
    #            (i.e. "tent structure")
    #  - q:      Constraints on loadings for quarterly variables
    #  - NQ:     Number of quarterly variables
    #  - i_idio: Logical. Gives index for monthly variables (1)
    #            and quarterly (0)
    #
    # Output:
    #  - A:   Transition matrix
    ...
    res = xBal.copy()   # Spline output equal to res
    # Later this is used for residuals
    ...
    for i in range(n_b):  # Loop for each block
        ...
        # Returns series index loading block i
        idx_i = np.where(blocks[:, i])[0]
        # Monthly series indicies for loaded blocks
        idx_iM = idx_i[idx_i < nM]
        # Quarterly series indicies for loaded blocks
        idx_iQ = idx_i[idx_i >= nM]

        # Returns eigenvector v w/largest eigenvalue d, CHECK: test if eig
        # values are the same in Matlab
        d, v = eig(np.cov(res[:, idx_iM], rowvar=False))
        e_idx = np.where(d == np.max(d))[0]
        d = d[e_idx]
        v = v[:, e_idx]

        # Flip sign for cleaner output. Gives equivalent results without
        # this section
        if np.sum(v) < 0:
            v = -v

        # For monthly series with loaded blocks (rows), replace with
        # eigenvector
        # This gives the loading
        C_i[idx_iM, 0:r_i] = v.copy()
        # Data projection for eigenvector direction
        f = np.matmul(res[:, idx_iM], v)
        ...
        # Quarterly-monthly aggregation scheme
        ...
        initV_i = np.reshape(
            np.matmul(
                np.linalg.inv(
                    np.eye((r_i*ppC)**2) - np.kron(A_i, A_i)
                ),
    ...
    for i in range(n_idio):  # Loop for monthly variables
        ...
        try:
            leadZero = np.max(np.where(np.arange(1, T+1) ==
                                       np.cumsum(np.isnan(res_i)))) + 1
        except ValueError:
            leadZero = None
        ...
        try:
            endZero = -(np.max(
                np.where(
                    np.arange(1, T+1) ==
                    np.cumsum(np.isnan(res_i[::-1])))[0]) + 1)
        except ValueError:
            endZero = None
        ...
        BM[i, i] = np.matmul(np.matmul(np.linalg.inv(
            np.matmul(res_i[:-1].T, res_i[:-1])), res_i[:-1].T), res_i[1:])
...
def EMstep(y, A, C, Q, R, Z_0, V_0, r, p, R_mat, q, nQ, i_idio, blocks):
    ...
    #
    #  Description:
    #    EMstep reestimates parameters based on the
    #    Estimation Maximization (EM) algorithm. This is a two-step procedure:
    ...
    #    blocks: Block structure for each series (i.e. for a series,
    #            the structure [1 0 0 1] indicates loadings on the
    ...
    # for a single Kalman filter sequence
    ...
    for i in range(num_blocks):
        # Loop for each block: factors are uncorrelated
        ...
        A_i[:r_i, :rp] = np.matmul(
            EZZ_FB[:r_i, :rp], np.linalg.inv(EZZ_BB[:rp, :rp]))

        # Equation 8: Covariance matrix of residuals of VA
        Q_i[:r_i, :r_i] = (EZZ[:r_i, :r_i] -
                           np.matmul(A_i[:r_i, :rp], EZZ_FB[:r_i, :rp].T))/T
        ...
        V_0_new[t_start:t_end, t_start:t_end] = Vsmooth[
            0,
    ...
    V_0_new[np.ix_(i_subset, i_subset)] = np.diag(
        np.diag(Vsmooth[0, i_subset, i_subset].copy()))
    ...
    # Cumulative number of monthly series
    c_i_idio = np.cumsum(i_idio)

    # Loop through unique loadings (e.g. [1 0 0 0], [1 1 0 0])
    for i in range(n_bl):

        bl_i = bl[[i], :].copy()
        # Total num of blocks loaded
        ...
        for t in range(T):
            ...
            denom += np.kron(
                np.matmul(
                    Zsmooth[bl_idxM_ind][:, [t+1]],
                    Zsmooth[bl_idxM_ind][:, [t+1]].T
            ...
            nom += np.matmul(
                ...
                Wt[:, i_idio_i],
                np.matmul(
                    Zsmooth[rp1+i_idio_ii][:, [t+1]],
                    Zsmooth[bl_idxM_ind][:, [t+1]].T
                ) + Vsmooth[t+1][rp1+i_idio_ii, :][:, bl_idxM_ind]
            )

        # POSSIBLE WEAK POINT FOUND: NEED TO TEST ON INDEXING AS NUMPY
        # DOES NOT MAINTAIN PROPER MATRIX FORM DEPENDING ON HOW ITS INDEXED

        # Eqn 13 BGR 2010
        vec_C = np.matmul(np.linalg.inv(denom),
                          nom.flatten('F').reshape((-1, 1)))

        # Place updated monthly results in output matrix
        C_new[np.ix_(idx_iM, bl_idxM_ind)] = vec_C.copy().reshape(
            (n_i, rs), order="F")  # CHECK: RESHAPE NEEDS TO BE VERIFIED
        ...
        for j in idx_iQ:
            ...
            i_idio_jQ = np.arange(rp1 + n_idio_M + 5 *
                                  (idx_jQ), rp1 + n_idio_M + 5*(idx_jQ+1))

            # Place quarterly values in output matrix
            V_0_new[np.ix_(i_idio_jQ, i_idio_jQ)] = Vsmooth[0][np.ix_(
                i_idio_jQ, i_idio_jQ)].copy()
            A_new[i_idio_jQ[0], i_idio_jQ[0]
                  ] = A_i[i_idio_jQ[0]-rp1, i_idio_jQ[0]-rp1].copy()
            Q_new[i_idio_jQ[0], i_idio_jQ[0]
                  ] = Q_i[i_idio_jQ[0]-rp1, i_idio_jQ[0]-rp1].copy()
            ...
            for t in range(T):
                ...
                Wt = np.diag(np.logical_not(
                    nanY[[j]][:, [t]]).astype(np.int64))

                # Intermediate steps in BGR equation 13
                denom += np.kron(
                    np.matmul(
                        Zsmooth[bl_idxQ_ind][:, [t+1]],
                ...
                nom -= np.matmul(
                    ...
                    np.matmul(
                        np.matmul(
                            np.array([[1, 2, 3, 2, 1]]),
                            Zsmooth[i_idio_jQ][:, [t+1]]
                        ),
                        Zsmooth[bl_idxQ_ind][:, [t+1]].T
                    ) + np.matmul(
                        np.array([[1, 2, 3, 2, 1]]),
                        Vsmooth[t+1][np.ix_(i_idio_jQ, bl_idxQ_ind)]
                    )
                )
    ...
    # Ensure non-zero measurement error.
...
def em_converged(loglik, prev_loglik, threshold=1e-4, check_decreased=1):

    # em_converged    checks whether EM algorithm has converged
    #
    #   Syntax:
    #     [converged, decrease] = em_converged(
    #           loglik, previous_loglik, threshold, check_increased)
    #
    #   Description:
    ...
    #
    #   Output:
    #     converged (numeric): Returns 1 if convergence criteria satisfied,
    #                          and 0 otherwise.
    ...
    if check_decreased == 1:
        if (loglik - prev_loglik) < -1e-3:
            print(
                f'******likelihood decreased from {prev_loglik} to {loglik}')
            decrease = 1

    # Check convergence criteria
    delta_loglik = np.abs(loglik - prev_loglik)  # Difference in loglik
    avg_loglik = (np.abs(loglik) + np.abs(prev_loglik) +
...
def SKF(Y, A, C, Q, R, Z_0, V_0):
    ...
    for t in range(nobs):
        ...
        if Y_t.shape[0] == 0:
            ...
            S["loglik"] = S["loglik"] + .5 * \
                (np.log(np.linalg.det(iF)) -
                 np.matmul(np.matmul(innov.T, iF), innov))[0, 0]
...
def FIS(A, S):
    ...
    #    with SKF(). See  page 154 of 'Forecasting, structural time series
    #    models and the Kalman filter' for more details (Harvey, 1990).
    #
    #  Input parameters:
    #    A: m-by-m transition matrix
    ...
    VmT_1_init = np.matmul(
        np.matmul(np.eye(m) - S["k_t"], A), np.squeeze(S["VmU"][nobs-1, :, :]))
    ...
    for t in range(nobs)[::-1]:
        ...
        S["ZmT"][:, [t]] = S["ZmU"][:, [t]] + \
            np.matmul(J_1, S["ZmT"][:, [t+1]] - np.matmul(A, S["ZmU"][:, [t]]))
