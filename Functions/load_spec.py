...
class load_spec():
    ...
    def __init__(self, filename):
        ...
        fldnms = ['SeriesID', 'SeriesName', 'Frequency',
        ...
        for field in fldnms:
            if field in raw.columns:
                ...
                raise ValueError(
                    f"{field} (column missing from model specification).")

        # Parse blocks
        jColBlock = list(
            raw.columns[raw.columns.str.contains("Block", case=False)])
        ...
        self.UnitsTransformed = np.array(
            [transformation[i] for i in self.Transformation])
