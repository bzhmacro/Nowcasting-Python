...
def update_nowcast(X_old, X_new, Time, Spec, Res, series, period, vintage_old,
                   vintage_new):

    if not type(vintage_old) == type(0):
        vintage_old = dt.strptime(
            vintage_old, '%Y-%m-%d').date().toordinal() + 366

    if not type(vintage_new) == type(0):
        vintage_new = dt.strptime(
            vintage_new, '%Y-%m-%d').date().toordinal() + 366
    ...
    future = np.array([
        (dt.fromordinal(Time[-1] - 366) + relativedelta(months=+ i)
         ).toordinal() + 366 for i in range(1, 13)])
    ...
    if t_nowcast.size == 0:
        s_err = 'Period is out of nowcasting horizon (up to one year ahead).'
        ValueError(s_err)
    ...
    y_old, _, _, _, _, _, _, _, _ = News_DFM(
        X_old, X_rev, Res, t_nowcast, i_series)

    # Display output
    y_rev, y_new, _, actual, forecast, weight, _, _, _ = News_DFM(
        X_rev, X_new, Res, t_nowcast, i_series)

    print("\n Nowcast Update: {}".format(dt.fromordinal(
        vintage_new-366).isoformat().split('T')[0]))
    print("\n Nowcast for: {} ({}), {}".format(
        Spec.SeriesName[i_series][0],
    ...
    if forecast.shape[0] == 0:
        ...
        print('              {} nowcast:              {:.5f}'.format(
            dt.fromordinal(vintage_old-366).isoformat().split('T')[0],
            y_old[0]))
        print('      Impact from data revisions:      {:.5f}'.format(
            impact_revisions[0]))
        print('       Impact from data releases:      {:.5f}'.format(
            np.nansum(news_table.Impact)))
        print('                                     +_________')
        print('                    Total impact:      {:.5f}'.format(
            impact_revisions[0] + np.nansum(news_table.Impact)))
        print('              {} nowcast:              {:.5f}'.format(
            dt.fromordinal(vintage_new-366).isoformat().split('T')[0],
            y_new[0]))
...
def News_DFM(X_old, X_new, Res, t_fcst, v_news):
    ...
    #   innov:       Difference between observed and predicted series values
    #                ("innovation")
    ...
    if ~np.isnan(X_new[t_fcst, v_news])[0]:
        ...
        # Indicator for missing--combine above information to
        #  single matrix where:
        ...
        if v_miss.shape[0] == 0:
            ...
            # CHECK: Why isn't this being used?
            ...
            for i in range(t_miss.shape[0]):
                ...
                X_new_norm = (X_new[t_miss[i], v_miss[i]] -
                              Mx[v_miss[i]])/Wx[v_miss[i]]
                X_sm_norm = (Res_old["X_sm"][
                    t_miss[i], v_miss[i]
                ] - Mx[v_miss[i]])/Wx[v_miss[i]]
            ...
            singlenews = np.zeros(
                (v_news.shape[0], np.max(t_miss) - np.min(t_miss)+1, N))
            actual = np.zeros((N, 1))
            ...
            for i in range(innov.shape[1]):
                ...
                forecast[v_miss[i], 0] = Res_old["X_sm"][t_miss[i],
                                                         v_miss[i]].copy()

                for j in range(v_news.shape[0]):
                    singlenews[j, t_miss[i] -
                               min(t_miss), v_miss[i]] = temp[j, 0, i].copy()
...
def para_const(X, P, lag):
    ...
    # Smoothed factor covariance for transition matrix
    Vs = Ss["VmT"][1:, :, :].copy()
    ...
    # Factors to series representation
    x_sm = np.matmul(Zsmooth[1:, :], C.T)
    # Standardized to unstandardized
