...
def load_data(datafile, Spec, sample=None):
    ...
    Python Version Notes:
        The original matlab function can load raw data from MATLAB formatted
         binary (.mat) file.
        However, in the Python version I have removed this feature.

        In transformData(), the formula_dict dictionary contains functions that
         transform the data. I could have done it in a similar fashion as the
         Matlab code, however I find it easier to read the code when going
         through the if and elif statements

        When converting dates to ordinal format, we need to add 366 days to
         match with Matlab date numeric values
    """
...
def transformData(Z, Time, Spec):
    ...
    formula_dict = {"lin": lambda x: x*2,
                    ...
                    "pch": lambda x: (np.append(
                        np.nan, x[t1+step::step]/x[t1:-1-t1:step]
                    ) - 1)*100,
                    "pc1": lambda x: ((x[12+t1::step]/x[t1:-12:step])-1)*100,
                    "pca": lambda x: (np.append(
                        np.nan, x[t1+step::step]/x[t1:-step:step]
                    )**(1/n) - 1)*100,
    ...
    for i in range(N):
        ...
        step = Freq_dict[freq]
        # assume monthly observations start at beginning of quarter
        # (subtracted 1 for indexing)
        t1 = step - 1
        n = step/12
        # number of years, needed to compute annual % changes
